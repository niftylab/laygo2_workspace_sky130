...
words=list()
for i in range(16):
    words.append(tlogic_adv['word_32bit_'+str(nf)+'x'].generate(name='word'+str(i*2)))
    words.append(tlogic_adv['word_32bit_'+str(nf)+'x'].generate(name='word'+str(i*2+1), transform='MX'))
decs = []
decs.append(tlogic_adv['dec2x4_'+str(nf)+'x'].generate(name='dec4'))
for i in range(4):
    decs.append(tlogic_adv['dec3x8_'+str(nf)+'x'].generate(name='dec8_'+str(i)))
for i in range(buffer_num):
    decs.append(tlogic_prim['buffer_'+str(16)+'x'].generate(name='buf_'+str(i)))
...
dsn.place(grid=pg, inst=decs, mn=mn_ref)
# 5. Create and place wires.
print("Create wires")
# WE
_mn = [r34.mn(words[0].pins['WE<0>'])[0], r34.mn(words[31].pins['WE<0>'])[1]]
