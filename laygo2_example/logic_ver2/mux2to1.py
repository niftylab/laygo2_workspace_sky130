...
# 3. Create istances.
   ...
   # in1, in2, in1, ip2: trackswaped -> S <=> D
...
# 5. Create and place wires.
   ...
   _mn = [r23.mn(ip1.pins['S'])[1], r23.mn(ip2.pins['S'])[0]]
   dsn.route(grid=r23, mn=_mn, via_tag=[False,False])
   _mn = [r23.mn(in1.pins['S'])[1], r23.mn(in2.pins['S'])[0]]
   dsn.route(grid=r23, mn=_mn, via_tag=[False,False])

   if nf == 2:
      _mn = [r23.mn(in4.pins['G'])[0], r23.mn(ip4.pins['G'])[0]]
      _track = [_mn[0][0]-1, None]
      r_int = dsn.route_via_track(grid = r23, mn=_mn, track =_track)
      _mn = [r23.mn(in2.pins['S'])[1], r23.mn(ip2.pins['S'])[1], r23.mn.bbox(r_int[-1])[0]]
   else:
      _mn = [r23.mn(in4.pins['G'])[0], r23.mn(ip4.pins['G'])[0]]
      dsn.route(grid=r23, mn=_mn, via_tag=[True,True])
      _mn = [r23.mn(in2.pins['S'])[1], r23.mn(ip2.pins['S'])[1], r23.mn(in4.pins['G'])[0]]
   _track = [None,int((r23.mn(in4.pins['G'])[0,1]+r23.mn(ip4.pins['G'])[1,1])/2)]
   dsn.route_via_track(grid=r23, mn=_mn, track=_track)
   dsn.via(grid=r23, mn=r23.mn(in2.pins['S'])[1])
   dsn.via(grid=r23, mn=r23.mn(ip2.pins['S'])[1])
...
# 8. Export to a template database file.
   ...
   db = laygo2.interface.yaml.export_template(nat_temp, filename=ref_dir_template+libname+'_templates.yaml', mode='append')
