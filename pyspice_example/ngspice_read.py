...
if __name__ == "__main__":
    ## plot out some informations about the ngspice files given by commandline
    for f in sys.argv[1:]:
        log = open("log_"+f.replace('raw','out').replace('sim_',''),'w')
        newtxt = ""
        newtxt += 'The file: "' + f + '" contains the following plots:'+'\n'
        for i,p in enumerate(ngspice_read(f).get_plots()):
            newtxt += '  Plot' + str(i) + 'with the attributes'+'\n'
            newtxt += '    Title: ' + p.title+'\n'
            newtxt += '    Date: '+ p.date+'\n'
            newtxt += '    Plotname: '+ p.plotname+'\n'
            newtxt += '    Plottype: ' + p.plottype+'\n'

            s = p.get_scalevector()
            newtxt += '    The Scale vector has the following properties:'+'\n'
            newtxt += '      Name: '+ s.name+'\n'
            newtxt += '      Type: '+ s.type+'\n'
            v = s.get_data()
            newtxt += '      Vector-Length: '+ str(len(v))+'\n'
            newtxt += '      Vector-Type: '+ str(v.dtype)+'\n'

            for j,d in enumerate(p.get_datavectors()):
                newtxt +='    Data vector'+ str(j)+ 'has the following properties:'+'\n'
                newtxt +='      Name: '+ d.name+'\n'
                newtxt +='      Type: '+ d.type+'\n'
                v = d.get_data()
                newtxt +='      Vector-Length: '+ str(len(v))+'\n'
